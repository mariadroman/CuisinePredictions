...
package object kddcup99 {
   ...
   * @return
   */
  def saveLinesToFile(rez: Iterable[String], path: String) = {
  ...
  def saveObjectToFile[T <: scala.AnyRef](that: T, path: String): Boolean = {

    // Create the HDFS file system handle
    val hdfs = FileSystem.get(new org.apache.hadoop.conf.Configuration())
    val writer = new ObjectOutputStream(new BufferedOutputStream(hdfs.create(new Path(path), true)))
    val result = Try(writer.writeObject(that)).isSuccess
    Try(writer.close)
    result
  }
  ...
  def loadObjectFromFile[T](path: String)(implicit mf: Manifest[T]): Try[T] = {
    ...
    // Create the HDFS file system handle
    val hdfs = FileSystem.get(new org.apache.hadoop.conf.Configuration())
    val reader = new ObjectInputStream(new BufferedInputStream(hdfs.open(new Path(path))))
