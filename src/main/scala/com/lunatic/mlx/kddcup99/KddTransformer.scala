...
/**
  ...
  */
case class KddTransformer(symboliColumns: List[Int], normAlgo: String = DataNormalizer.L2NormV1,
  ...
  override def learn(input: RDD[Array[String]]): EducatedTransformer[Array[String], Vector] = {
    ...
    val rc = removableColumns.getOrElse(columnsWithEqualValues(input).toList)
    ...
    KddTransformer(symboliColumns, normAlgo, Some(rc), Some(colRemover), Some(hasher), Some(vectorizer), Some(customNormalizer), Some(l1Normalizer))
  }
