...
//    predict
  ...
  def predict(implicit sc: SparkContext, appConf: Configuration): Unit = {
    ...
    for (
      ...
      val predFile = stubFilePath(params) + s"_${file._2}.prediction"
  ...
  def predictAndSaveSmallSample(implicit sc: SparkContext, appConf: Configuration): Unit = {
    ...
    for (
      ...
      def predictions = predict(model, transformer, unlabeledData, threshold)
      val results = predictionResults(labels.zip(predictions), threshold)
      val sampleFile = stubFilePath(params) + s"_${file._2}.R_sample"
      val sample = predictions.map{case (isAnom, clust, dist, lp, vec) =>
        clust + "," + vec.toArray.mkString(",")
      }.collect
  ...
  def predictionResults(predictions: RDD[(String, (Boolean, Int, Double, Array[String], Vector))], threshold: Double) = {
    ...
    val resLines = "" +:
      ...
      "" +:
      f"| ${"Label"}%-20s | ${"Clust"}%-5s | ${"Distance"}%-12s | Input Vector | Normalized Vector" +:
